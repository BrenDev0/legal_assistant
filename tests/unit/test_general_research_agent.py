...
@pytest.fixture
def general_legal_researcher(mock_prompt_service, mock_llm_service):
...
@pytest.mark.asyncio
async def test_interact_employment_law_query(general_legal_researcher, sample_state, mock_prompt_service, mock_llm_service):
    ...
    # Verify method calls - Fixed to match actual agent parameters
    mock_llm_service.get_llm.assert_called_once_with(temperature=0.1, max_tokens=2500)
...
@pytest.mark.asyncio
async def test_interact_corporate_law_query(general_legal_researcher, mock_prompt_service, mock_llm_service):
    state = State({
...
@pytest.mark.asyncio
async def test_interact_tax_law_query(general_legal_researcher, mock_prompt_service, mock_llm_service):
    state = State({
    ...
    # Verify LLM parameters - Fixed to match actual agent
    mock_llm_service.get_llm.assert_called_once_with(temperature=0.1, max_tokens=2500)

@pytest.mark.asyncio
async def test_interact_empty_response_handling(general_legal_researcher, mock_prompt_service, mock_llm_service):
    state = State({
...
@pytest.mark.asyncio
async def test_prompt_template_system_message_content(general_legal_researcher, mock_prompt_service, mock_llm_service):
    state = State({
        "input": "Test query",
        "agent_id": "test_agent",
        "user_id": "test_user"
...
@pytest.mark.asyncio
async def test_interact_with_context_enabled(general_legal_researcher, mock_prompt_service, mock_llm_service):
    state = State({
